...
const isRestaurantOpen = (openingTime: string, closingTime: string): boolean => {
  ...
  const [openingHour, openingMinute] = openingTime.split(':').slice(0, 2).map(Number);
  const [closingHour, closingMinute] = closingTime.split(':').slice(0, 2).map(Number);
...
const transformApiRestaurantToRestaurantData = (restaurant: ApiRestaurant): RestaurantData => {
  ...
  const status = isRestaurantOpen(formattedOpeningTime, formattedClosingTime) ? 'Ouvert' : 'Fermé';
...
export const transformRestaurantsApiResponse = (response: RestaurantsApiResponse): RestaurantData[] => {
