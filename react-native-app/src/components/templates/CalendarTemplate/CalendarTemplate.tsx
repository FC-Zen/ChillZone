...
export const CalendarTemplate: React.FC<CalendarTemplateProps> = ({
  ...
  // Formatte le temps pour l'affichage dans le modal de détails de cours
  // Exemple: start_time : 2021-10-01T08:30:00.000Z
  // end_time : 2021-10-01T10:30:00.000Z
  // 08:30 - 10:30
  const formatedTime = (start_time: Date, end_time: Date) => {
    let start = start_time.toLocaleTimeString('fr-FR', {
      hour: '2-digit',
      minute: '2-digit',
    });
    let end = end_time.toLocaleTimeString('fr-FR', {
      hour: '2-digit',
      minute: '2-digit',
    });
    return `${start} - ${end}`;
    ...
    <>
      ...
      <View style={styles.container}>
        <View style={styles.calendar}>
          <Calendar
            ...
            onPressEvent={(event) => {
              setCourseModal(true);
              setActualEvent(events.find((e) => e.id === event.id) || null);
            }}
      ...
      <CourseDetailsModal
        ...
        courseDetails={{
          ...
          time: formatedTime(
            actualEvent?.start_time || new Date(),
            actualEvent?.end_time || new Date()
          ),
          room: actualEvent?.location || '',
          instructor:
            actualEvent?.professor.toString().replaceAll(',', ', ') || '',
        }}
      />
      <AdeModal
        isOpen={adeModal}
